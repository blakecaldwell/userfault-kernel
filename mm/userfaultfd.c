		    ...
		    unsigned long len, __u64 mode)
{
	 ...
	 * Sanitize the command parameters:
	 */
	WARN_ON(src_start & ~PAGE_MASK);
	WARN_ON(dst_start & ~PAGE_MASK);
	WARN_ON(len & ~PAGE_MASK);

	/* Does the address range wrap, or is the span zero-sized? */
	WARN_ON(src_start + len <= src_start);
	WARN_ON(dst_start + len <= dst_start);
	...
	for (src_addr = src_start, dst_addr = dst_start;
		...
		WARN_ON(dst_addr >= dst_start + len);
		...
		if (ptl) {
			...
			if (!thp_aligned || (src_addr & ~HPAGE_PMD_MASK) ||
				...
				WARN_ON(dst_addr & ~HPAGE_PMD_MASK);
		...
		WARN_ON(pmd_none(*dst_pmd));
		WARN_ON(pmd_none(*src_pmd));
		WARN_ON(pmd_trans_huge(*dst_pmd));
		WARN_ON(pmd_trans_huge(*src_pmd));
