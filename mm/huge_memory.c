			 ...
			 unsigned long src_addr)
{
	...
	WARN_ON(!pmd_trans_huge(src_pmdval));
	WARN_ON(!pmd_none(dst_pmdval));
	WARN_ON(!spin_is_locked(src_ptl));
	WARN_ON(!rwsem_is_locked(&src_mm->mmap_sem));
	WARN_ON(!rwsem_is_locked(&dst_mm->mmap_sem));

	src_page = pmd_page(src_pmdval);
	WARN_ON(!PageHead(src_page));
